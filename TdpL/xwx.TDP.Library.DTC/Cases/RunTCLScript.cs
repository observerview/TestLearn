...
namespace xwx.TDP.Library.DTC.Cases
{
    [Category("FPGA控制"),
     DisplayName("执行tcl脚本"),
     Description("执行tcl脚本来实现跟fpga各种功能交互，这个case为配置tcl脚本执行。"),
    ]
    public class RunTCLScript : CoreCaseCommon
    {
        ...
        private class ParameterSetting : ConfigBase
        {
            ...
            private string _tclScriptFilePath = @"C:\AutoTest\Data\DTC\FPGA\demo_script.tcl";
            [Category("A.参数设置组"),
             DisplayName("A02.TCL脚本"),
             Description("TCL脚本完整路径，包括文件名"),
             DefaultValue(typeof(string), @"C:\AutoTest\Data\DTC\FPGA\demo_script.tcl"),
            ]
            public string TclScriptFilePath
            {
                get { return _tclScriptFilePath; }
                set { _tclScriptFilePath = value; }
            }
            ...
            //
            private string _displayName = "执行tcl脚本";

            [DisplayName("Z01. 显示名称")]
            [Category("Z. 测试case通用配置")]
            [DefaultValue("执行tcl脚本")]
            [Description("执行tcl脚本")]
            public string DisplayName
            {
                ...
                set
                {
                    _displayName = (string.IsNullOrEmpty(value) ? "执行tcl脚本" : value);
                }
            }
        }
        #endregion
        ...
        public RunTCLScript() : this("执行tcl脚本")
        {
        }

        public RunTCLScript(string displayName)
        {
        ...
        public override ExecOkError Exec()
        {
            ...
            using (Process process = new Process())
            {
                ...
                using (var streamWriter = process.StandardInput)
                {
                    ...
                    {
                        // 发送vivado.bat命令并执行TCL脚本
                        streamWriter.WriteLine($"\"{vivadoPath}\" -mode tcl -source \"{_parameterSetting.TclScriptFilePath}\"");
