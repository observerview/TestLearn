...
{
    [Category("电源控制"),
     DisplayName("电源D1790上电,并量读电流判定"),
     Description("电源D1790上电,并量读电流，电流可设置上下限"),
    ]
    public class D1790_PowerResetAndReadCurrent : CoreCaseCommon
    {
        ...
        private class ParameterSetting : ConfigBase
        {
            ...
            [Category("A.程控电源设置"),
             ...
             Description("设置对应的程控电源通道,对于只有一个通道的电源，设0，D1790该参数设置0；"),
            ...
            private double _psuVoltage = 42;
            [Category("A.程控电源设置"),
             DisplayName("A02.输出电压(V)"),
             Description("设置程控电源输出电压"),
             DefaultValue(typeof(double), "42")
            ]
        ...
        public override ExecOkError Exec()
        {
            ...
            if (!_psu1790.SetCurrentProtectStatus(DH1790Psu.OnOff.On))
            {
                LogInfo(LogInfoType.Error, "设置电源过流保护!");
                return ExecOkError.Error;
            }
            ...
            if (!_psu1790.GetCurrent(_parameterSetting.PathNumber, out _current))
            {
